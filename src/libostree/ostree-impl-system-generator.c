...
/* Generate a drop-in for a systemd-cryptsetup@ instance to preserve it
 * across soft-reboot. Written to /run/systemd/system/ which has higher
 * precedence than /run/systemd/generator/ where the cryptsetup-generator
 * creates the instance unit with Conflicts=umount.target.
 *
 * @luks_name: The LUKS device name (e.g., "myluksdev")
 */
static gboolean
generate_cryptsetup_runtime_dropin (const char *luks_name, GError **error)
{
  GCancellable *cancellable = NULL;
  static const char *runtime_dir = "/run/systemd/system";

  /* Build the drop-in directory path */
  g_autofree char *dropin_dir
      = g_strdup_printf ("%s/systemd-cryptsetup@%s.service.d", runtime_dir, luks_name);

  if (g_mkdir_with_parents (dropin_dir, 0755) != 0)
    return glnx_throw_errno_prefix (error, "Failed to create %s", dropin_dir);

  g_autofree char *dropin_path = g_strdup_printf ("%s/ostree-softreboot.conf", dropin_dir);

  /* Write the drop-in that clears Conflicts=umount.target and
   * Before=umount.target set by systemd-cryptsetup-generator.
   *
   * We use /run/systemd/system/ because it has higher precedence than
   * /run/systemd/generator/ where the instance unit lives. A template
   * drop-in at /usr/lib/systemd/system/ cannot override explicit values
   * in the generated instance unit because /run/systemd/generator/ has
   * higher precedence than /usr/lib/systemd/system/.
   *
   * DefaultDependencies=no removes implicit Conflicts=shutdown.target.
   * Conflicts= (empty) resets the explicit Conflicts=umount.target.
   * Before= (empty) resets the explicit Before=umount.target.
   *
   * On real shutdown, systemd-shutdown(8) handles device teardown directly.
   */
  g_autofree char *contents
      = g_strdup_printf ("##\n# Automatically generated by ostree-system-generator\n"
                         "# Preserve cryptsetup for %s across soft-reboot\n"
  ...
  if (!g_file_set_contents (dropin_path, contents, -1, error))
    return FALSE;

  return TRUE;
}

/* Generate a service that ensures /etc/crypttab has x-initrd.attach for
 * LUKS devices backing /sysroot. This prevents systemd-cryptsetup-generator
 * from adding Conflicts=umount.target to the cryptsetup service, which would
 * tear down the LUKS device during soft-reboot.
 *
 * On non-ostree systems, Conflicts=umount.target is harmless because the
 * kernel root mount (-.mount, Perpetual=yes) pins the LUKS device — cryptsetup
 * detach fails with EBUSY. On ostree, /sysroot is a regular mount (not the
 * kernel root), so the cascade can actually tear it down. Adding x-initrd.attach
 * to crypttab tells the cryptsetup generator to not add Conflicts=umount.target.
 *
 * We generate a oneshot service because generators cannot modify /etc/crypttab
 * directly (they run in a read-only context) and cannot trigger daemon-reload.
 * The service runs early on boot, modifies crypttab if needed, and reloads
 * systemd so the cryptsetup generator re-runs with the updated crypttab.
 */
static gboolean
generate_luks_softreboot_service (int normal_dir_dfd, GError **error)
{
  GCancellable *cancellable = NULL;
  ...
  /* The service:
   * - Only runs if /etc/crypttab exists and doesn't already have x-initrd.attach
   * - Appends x-initrd.attach to each crypttab entry's options field
   * - Triggers daemon-reload so systemd-cryptsetup-generator re-runs
   *
   * crypttab format: name device keyfile options
   * If options is "none" or "-", we replace with "x-initrd.attach"
   * Otherwise we append ",x-initrd.attach" to existing options
   */
  if (!g_output_stream_printf (
          outstream, &bytes_written, cancellable, error,
          "##\n# Automatically generated by ostree-system-generator\n"
          "# Ensures LUKS devices have x-initrd.attach for soft-reboot support.\n"
          "# See https://github.com/ostreedev/ostree/pull/3571\n##\n\n"
          "[Unit]\n"
          "Description=Ensure LUKS crypttab has x-initrd.attach for ostree soft-reboot\n"
          "DefaultDependencies=no\n"
          "ConditionPathExists=/etc/crypttab\n"
          "Before=cryptsetup.target local-fs-pre.target\n"
          "After=-.mount\n\n"
          "[Service]\n"
          "Type=oneshot\n"
          "RemainAfterExit=yes\n"
          "# Skip if x-initrd.attach is already present\n"
          "ExecCondition=/usr/bin/bash -c '! /usr/bin/grep -q x-initrd.attach /etc/crypttab'\n"
          "# Add x-initrd.attach to crypttab and reload systemd\n"
          "ExecStart=/usr/lib/ostree/ostree-crypttab-softreboot\n"))
    return FALSE;
  ...
  if (!glnx_link_tmpfile_at (&tmpf, GLNX_LINK_TMPFILE_NOREPLACE, normal_dir_dfd,
                             "ostree-luks-softreboot.service", error))
    return FALSE;

  /* Want this service from cryptsetup-pre.target so it runs before
   * the cryptsetup generator's units are started */
  if (!glnx_shutil_mkdir_p_at (normal_dir_dfd, "cryptsetup-pre.target.wants", 0755, cancellable,
                               error))
    return FALSE;
  if (symlinkat ("../ostree-luks-softreboot.service", normal_dir_dfd,
                 "cryptsetup-pre.target.wants/ostree-luks-softreboot.service")
      < 0)
    {
      if (errno != EEXIST)
        return glnx_throw_errno_prefix (error, "symlinkat");
 ...
 * and if LUKS is in use, generate a service to fix crypttab.
 *
 ...
 */
static gboolean
sysroot_mount_generator (const char *normal_dir, const char *cmdline, GError **error)
{
  glnx_autofd int normal_dir_dfd = -1;
  ...
  /* If LUKS is in use (rd.luks.name= on cmdline), generate a service
   * that ensures x-initrd.attach is in /etc/crypttab. */
  if (strstr (cmdline, "rd.luks.name=") != NULL)
    {
      if (!generate_luks_softreboot_service (normal_dir_dfd, error))
        return FALSE;
    }
                               ...
                               GError **error)
{
  ...
  if (!require_internal_units (normal_dir, early_dir, late_dir, error))
    return FALSE;
  if (!sysroot_mount_generator (normal_dir, cmdline, error))
    return FALSE;
